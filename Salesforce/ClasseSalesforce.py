...
class SalesForce:
    ...
    def get_table(self, table_name, select = '', where = None, limit = None, label = True, bar = True, show_na = False, print_query = False):
        '''
        ...
        data = self.get_query(q1+' LIMIT 1')
        ...
        # pd.DataFrame com os dados da query
        df = self.get_query(q1)
    ...
    def get_table_iter(self, obj = str, select = '', iter_serie = pd.Series, lookup_col = '', normalize_txt = False, step = 200, print_query = False):
            ...
            if len(iter_serie) == 0:
                print('iter_seria está vazio!')
                return
            
            iter_serie = iter_serie.fillna('')
            # - Carrega lista de valores para o loop
            if normalize_txt:
              leads_from_vendas = list(iter_serie.str.lower().drop_duplicates().dropna().values)
            else:
              leads_from_vendas = list(iter_serie.drop_duplicates().dropna().values)
            ...
            for x in tqdm(range(0, len(leads_from_vendas), passo), colour='#2CD5E4', leave = False, desc = f'{obj}'):
              ...
              if type(select) == str and select != '':
                  select = select.split(',')
              
            df_aux.columns = select
            ls_lf.append(df_aux)
