# -*- coding: utf-8 -*-
"""
Created on Tue Jan 30 14:30:07 2024
...
# - Chama API    
    ...
    def chamar_api2(self, params_s = None, endpoint = None):
...
# Transforma dados em df    
            ...
            if pbar == True:
                ...
                if len(all_sales_data) >0: # Evita divisão por zero!
                    if switch == 0: # Garante que o total_runs será definido apenas uma vez!
                        pbar = tqdm(total = int(page_info['total_results']/100)+1, colour = '#2CD0E5', leave = False)
    ...
    def get_transaction_hm(self, transactions, return_not_found = False):
        ...
        dfs_t = []
        not_found = []
        for t in tqdm(transactions, leave = True, desc = 'Transactions..'):
            try:
        ...
        if len(dfs_t) > 1:
            ...
            df.loc[:,[c for c in df.columns if 'date' in c]] = df.loc[:,[c for c in df.columns if 'date' in c]].apply(lambda x: pd.to_datetime(x))
...
# - Utilitárias
    ...
    def get_df_from_json_serie(self, serie = pd.Series):
        df_aux = pd.concat([pd.DataFrame([x]) for x in tqdm(serie, leave = False)])
...
# - Main Function (Executa loop na paginação)
    def get_sales_hm(self, start_date=None, end_date= None, transaction = '', formato = '%Y-%m-%d', endpoint = None, pbar = True):
        ...
        try:
            ...
            for c in tqdm(lst_col_dict, leave = False):
